...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dJdb = torch.ones((N, 1), dtype=dx.dtype, device=dout.device)
        db = torch.mm(dout.t(), dJdb).squeeze(1)
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        z, cache1 = Linear.forward(x,w,b)
        out, cache2 = ReLU.forward(z)
        cache = (cache1,cache2)
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
    ...
    def backward(dout, cache):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        cache_layer, cache_relu = cache
        dz = ReLU.backward(dout,cache_relu)
        dx,dw,db = Linear.backward(dz,cache_layer)
        ######################################################################
        #                END OF YOUR CODE                                    #
        ######################################################################
        return dx, dw, db
...
def softmax_loss(x, y):
    ...
    # Used e^(x_k-x_max)/sum(e^(x_i-x_max),i=1:C) for less computational cost
    N = x.shape[0]
    smax_out = torch.exp(x - x.max(axis=1, keepdims=True)[0])
    smax_out /= smax_out.sum(axis=1, keepdims=True)
    
    # Applied the cross entropy cost function:
    loss = -torch.log(smax_out[torch.arange(N),y]).sum() / N
    dx = smax_out
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        w1 = torch.normal(0, weight_scale, dtype = dtype, size = (input_dim, hidden_dim)).to(device)
        b1 = torch.zeros(hidden_dim, dtype = dtype, device = device)
        w2 = torch.normal(0, weight_scale, dtype = dtype, size = (hidden_dim, num_classes)).to(device)
    ...
    def loss(self, X, y=None):
        ...
        loss += self.reg * (torch.norm(self.params['W1'])**2 + torch.norm(self.params['W2'])**2)

        # Layer 1 Loss Backpropagation
        dlayer1, dw2, db2 = Linear.backward(dout, cache_layer2)
        dw2 =  dw2 + 2 * self.reg * self.params['W2']
        
        # Input Loss Backpropagation
        dx, dw1, db1 = Linear_ReLU.backward(dlayer1, cache_layer1)
        dw1 =  dw1 + 2 * self.reg * self.params['W1']
        
        # Saving the gradians in the grad dict
        grads['W1'] = dw1
        grads['W2'] = dw2
        grads['b1'] = db1
        grads['b2'] = db2
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # Total number of parameters to initialize
        all_parms = [input_dim] + hidden_dims + [num_classes]

        # Applying the initialization to all parameters
        for i in range(self.num_layers):
          w0_i = torch.normal(0, weight_scale, dtype = dtype,size = (all_parms[i], all_parms[i+1])).to(device)
          b0_i = torch.zeros(all_parms[i+1], dtype = dtype, device = device)
          self.params[f'W{i+1}'] = w0_i
          self.params[f'b{i+1}'] = b0_i
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        out = X
        cache_all = []
        # Running the feedforward network (in hidden layers we use ReLu activation function)
        for i in range(self.num_layers):
          if i == self.num_layers - 1:
              scores, cache = Linear.forward(out, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
              cache_all.append(cache)
          else:
              # Check if we use dropout for forward stages of hidden layers and apply it
              ...
              if self.use_dropout:
                  ...
                  cache_all.append((cache, dropout_cache))
              else:
                  cache_all.append(cache)
        ...
        # Find the loss of the softmax layer
        loss, dout = softmax_loss(scores, y)

        # Find the loss in the latest layer before the softmax and applying the L2 regularization
        for i in range(self.num_layers):
            loss += self.reg * torch.sum(self.params[f'W{i+1}'] * self.params[f'W{i+1}'])
        
        # Update the weights and biases with respect to the gradient values
        for i in range(self.num_layers-1,-1,-1):
            if i == self.num_layers - 1:
                ...
                # Check if we use dropout for backward stages of hidden layers and apply it
                if self.use_dropout:
                    cache, dropout_cache = cache_all[i]
...
def sgd_momentum(w, dw, config=None):
    ...
    # Update the velocity and the weights interms of last v and current dw
...
def rmsprop(w, dw, config=None):
    ...
    # Updating the cache and the weights by using RMSprop method that is applying adaptive learning rates
...
def adam(w, dw, config=None):
    ...
    # The ADAM update rule that uses bias correction mechanism for adaptive learning rates of SGD
    # Updating the time index
    config['t'] = config['t']+1

    # Finding the momentum term by beta1 rate
    config['m'] = config['beta1']*config['m'] + (1-config['beta1'])*dw
    m = config['m'] / (1-config['beta1']**config['t'])

    # Finding the velocity term by beta2 rate
    config['v'] = config['beta2']*config['v'] + (1-config['beta2'])*(dw**2)
    v = config['v'] / (1-config['beta2']**config['t'])

    # Update the weights by using calculated m,v parameters and our constant learning rate
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            # Using dropout mask (U) to drop some random neurons' outputs (divided by 1-p for inverted dropout)
            ...
            # Just pass the data (Because we've used the inverted dropout in training phase!)
            out = x
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
            ...
            # Just drop the ones that didn't participate in the forward stage
            dx = dout * mask
